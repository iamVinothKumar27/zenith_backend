...
def utc_iso():
    """Timezone-aware UTC ISO string (includes offset)."""
...
def _sanitize_topic(topic: str) -> str:
    ...
    if not t:
        return ""
    if t.lower() in {"all", "all topics", "all_topic", "any", "mixed", "everything"}:
        return ""
    return t

def _ensure_iso_tz(dt_str):
    """If a stored ISO datetime string has no timezone, treat it as UTC."""
    if not dt_str or not isinstance(dt_str, str):
        return dt_str
    t = dt_str.strip()
    # Already has timezone offset or Z
    if re.search(r'(Z|[+-]\d{2}:\d{2})$', t):
        return t
    return t + "+00:00"
...
def _default_proctoring(limit=3):
    try:
...
def _topic_matches(value, topic: str):
    aliases = _topic_aliases(topic)
    if not aliases:
        return True
    if isinstance(value, list):
        hay = " ".join(str(v or "") for v in value).lower()
    else:
        hay = str(value or "").lower()
    return any(a in hay for a in aliases)


def _proctoring_public(data):
    base = _default_proctoring()
    src = data if isinstance(data, dict) else {}
    out = {**base, **src}
    try:
        out["violations"] = int(src.get("violations") or 0)
    except Exception:
        out["violations"] = 0
    try:
        out["violation_limit"] = int(src.get("violation_limit") or base["violation_limit"])
    except Exception:
        out["violation_limit"] = base["violation_limit"]

    warnings = src.get("warnings")
    if not isinstance(warnings, list):
        warnings = []
    out["warnings"] = [w for w in warnings if isinstance(w, dict)]
    ...
    out["auto_submitted"] = bool(src.get("auto_submitted"))
    out["auto_submit_reason"] = str(src.get("auto_submit_reason") or "")
    lva = src.get("last_violation_at")
    out["last_violation_at"] = _ensure_iso_tz(lva) if isinstance(lva, str) else lva
    return out
...
MAIL_FROM_MOCKTEST = (os.getenv("MAIL_FROM_MOCKTEST") or os.getenv("MAIL_FROM_TESTS") or "zenith-tests@zenithlearning.site").strip()
...
def _pick_from(kind: str = "") -> str:
    ...
    if k in ("mocktest", "mock-tests", "tests", "mock_test", "practicetest", "practice_test"):
...
def _pick_sender_name(kind: str = "") -> str:
    k = (kind or "").strip().lower()
    if k in ("auth", "authentication", "login"):
        return "zenith-authentication"
    if k in ("courses", "course", "quiz"):
        return "zenith-courses"
    if k in ("profile",):
        return "zenith-profile"
    if k in ("admin",):
        return "zenith-admin"
    if k in ("contact",):
        return "zenith-contact"
    if k in ("mocktest", "mock-tests", "tests", "mock_test", "practicetest", "practice_test"):
        return "zenith-tests"
    if k in ("ats", "ats-intelligence", "ats_intelligence"):
        return "zenith-ats"
...
def _sanitize_test_name_for_email(name: str, *, is_practice: bool = False) -> str:
    raw = str(name or "").strip()
    if not raw:
        return "Practice Test" if is_practice else "Mock Test"
    if is_practice:
        raw = re.sub(r"\bproctoring\b", "", raw, flags=re.I)
        raw = re.sub(r"\s{2,}", " ", raw).strip(" -•_")
        return raw or "Practice Test"
    return raw


def _format_proctoring_warning_item(item) -> str:
    if isinstance(item, dict):
        msg = str(item.get("message") or item.get("warning") or item.get("type") or "Warning recorded").strip()
        at = str(item.get("at_ist") or item.get("at") or item.get("time") or "").strip()
        return f"{msg} ({at})" if at else msg
    return str(item or "").strip()


def _format_proctoring_event_item(item) -> tuple[str, str]:
    if isinstance(item, dict):
        msg = str(item.get("message") or item.get("warning") or item.get("type") or "Violation recorded").strip()
        at = str(item.get("at_ist") or item.get("at") or item.get("time") or "").strip()
        return msg, at
...
def _proctoring_details_html(proctoring: dict) -> str:
    p = proctoring if isinstance(proctoring, dict) else {}
    if not p or not p.get("enabled"):
        return ""

    violations = int(p.get("violations") or 0)
    limit = int(p.get("violation_limit") or 0)
    warnings = []
    for item in (p.get("warnings") or []):
        formatted = _format_proctoring_warning_item(item)
        if formatted:
            warnings.append(formatted)
    events = p.get("events") or []
    reason = str(p.get("auto_submit_reason") or "").strip()

    event_lines = []
    for ev in events[-5:]:
        msg, at = _format_proctoring_event_item(ev)
        if not msg:
            continue
        line = _html_escape(msg)
        if at:
            line += f" <span style='color:#6b7280;'>(at {_html_escape(at)})</span>"
        event_lines.append(f"<li>{line}</li>")

    parts = [
        "<hr/>",
        "<p><b>🛡️ Proctoring Details</b></p>",
        f"<ul><li><b>Violations:</b> {violations}/{limit or 3}</li>"
        + (f"<li><b>Status:</b> Auto-submitted</li>" if p.get("auto_submitted") else "<li><b>Status:</b> Completed without auto-submit</li>")
        + (f"<li><b>Reason:</b> {_html_escape(reason)}</li>" if reason else "")
        + "</ul>",
    ]
    if warnings:
        parts.append("<p><b>Warnings</b></p><ul>" + "".join(f"<li>{_html_escape(w)}</li>" for w in warnings[-5:]) + "</ul>")
    if event_lines:
        parts.append("<p><b>Recent Proctoring Events</b></p><ul>" + "".join(event_lines) + "</ul>")
    return "".join(parts)


def _proctoring_details_text(proctoring: dict) -> str:
    p = proctoring if isinstance(proctoring, dict) else {}
    if not p or not p.get("enabled"):
        return ""
    violations = int(p.get("violations") or 0)
    ...
    lines = [
        "",
        "Proctoring Details:",
        f"Violations: {violations}/{limit or 3}",
        f"Status: {'Auto-submitted' if p.get('auto_submitted') else 'Completed without auto-submit'}",
    ]
    if reason:
    ...
    if events:
        lines.append("Recent Proctoring Events:")
        for ev in events[-5:]:
            msg, at = _format_proctoring_event_item(ev)
            if not msg:
                continue
            lines.append(f"- {msg}" + (f" (at {at})" if at else ""))
    return "\n".join(lines)
...
def _send_email_sync_smtp(
    ...
    *,
    ...
    msg["From"] = f"{_pick_sender_name(kind)} <{sender}>"
    msg["To"] = to_email
    msg["Subject"] = subject
    if reply_to:
...
def _send_email_sync_sendgrid(to_email: str, subject: str, html_body: str, text_body: str = "", *, kind: str = "") -> bool:
    ...
    payload = {
        ...
        "from": {"email": from_email, "name": _pick_sender_name(kind)},
        "content": [
...
def _send_email_sync_mailgun(
    ...
    *,
    ...
    data = {
        "from": f"{_pick_sender_name(kind)} <{from_email}>",
        "to": [to_email],
        "subject": subject,
        "text": text_body or " ",
        "html": html_body,
    ...
    *,
    ...
    def _job():
        ...
        if provider == "sendgrid":
            if _send_email_sync_sendgrid(to_email, subject, html_body, text_body, kind=kind):
                return
            provider = "mailgun" if _mailgun_ready() else "smtp"

        if provider == "mailgun":
            if _send_email_sync_mailgun(to_email, subject, html_body, text_body, kind=kind, from_email=from_email, reply_to=reply_to):
                return
            provider = "smtp"

        _send_email_sync_smtp(to_email, subject, html_body, text_body, kind=kind, from_email=from_email, reply_to=reply_to)
...
def _brand_email(
    ...
    **_ignore,
    ...
    logo_text = _pick_sender_name(kind)
            ...
            <tr>
              <td style="padding:0 24px 22px 24px;">
                <div style="border-top:1px solid #e5e7eb;padding-top:14px;font-size:12px;color:#6b7280;line-height:1.6;">
                  ...
                  <span style="color:#9ca3af;">© {year} {logo_text}. All rights reserved.</span>
                </div>
              </td>
            </tr>
          </table>
...
def auth_firebase():
    ...
    lookup = {"email": email_norm} if email_norm else {"uid": uid}
    existing_user = db.users.find_one(lookup, {"_id": 0, "avatarFileId": 1, "photoLocalURL": 1, "photoURL": 1}) or {}
    ...
    # Keep custom uploaded avatar intact. When the user has no custom avatar, prefer the Google/Firebase photo URL.
    if existing_user.get("avatarFileId"):
        set_doc["photoURL"] = existing_user.get("photoURL") or photoURL
    else:
        set_doc["photoLocalURL"] = ""
        set_doc["avatarFileId"] = ""
        set_doc["photoURL"] = photoURL
...
def profile_me_get():
    ...
    if err:
        ...
        return jsonify({"error": msg}), code
    db = get_db()
    doc = db.users.find_one({"uid": user["uid"]}, {"_id": 0}) or {}

    auth_photo_url = (user.get("picture") or user.get("photoURL") or "").strip()
    avatar_file_id = str(doc.get("avatarFileId") or "").strip()
    if not avatar_file_id:
        doc["photoLocalURL"] = ""
        if auth_photo_url:
            doc["photoURL"] = (doc.get("photoURL") or auth_photo_url).strip()
    doc["authPhotoURL"] = auth_photo_url
...
def profile_photo_delete():
    ...
    now = datetime.now(timezone.utc)
    fallback_photo_url = (user.get("picture") or user.get("photoURL") or "").strip()
    db.users.update_one(
        {"uid": uid},
        {"$set": {"photoLocalURL": "", "photoURL": fallback_photo_url, "avatarFileId": "", "updatedAt": now}},
        upsert=True,
    )
...
@app.route("/admin/practicetest-analytics", methods=["GET"])
def admin_practicetest_analytics():
    """Admin-only aggregate analytics for PRACTICE test sessions."""
    _, err = require_admin()
    if err:
        return jsonify({"error": err[0]}), err[1]
    ...
    try:
        db = get_db()
        col = _mocktest_sessions_col(db)
        docs = list(col.find({"kind": "practice"}).sort("created_at", -1).limit(2000))
...
@app.route("/admin/practicetest-analytics/users", methods=["GET"])
def admin_practicetest_analytics_users():
    """Admin-only user-wise analytics for PRACTICE test sessions."""
    _, err = require_admin()
    if err:
        return jsonify({"error": err[0]}), err[1]
    ...
    try:
        db = get_db()
        col = _mocktest_sessions_col(db)
        docs = list(col.find({"kind": "practice"}).sort("created_at", -1).limit(5000))
        ...
        for d in docs:
            ...
            try:
                if a["lastAttempt"] is None:
                    ...
                    if isinstance(created, str) and isinstance(a["lastAttempt"], str) and created > a["lastAttempt"]:
                        a["lastAttempt"] = created
            except Exception:
                pass
...
def admin_user_mocktest_detail(uid, session_id):
    ...
    out = {
        ...
        "proctoring": _proctoring_public(doc.get("proctoring") or {}),
...
@app.get("/admin/user/<uid>/practicetests")
def admin_user_practicetests(uid):
    """Recent practice test sessions for a single user."""
    _, err = require_admin()
    if err:
        msg, code = err
        return jsonify({"error": msg}), code
    ...
    db = get_db()
    col = _mocktest_sessions_col(db)
    docs = list(col.find({"uid": uid, "kind": "practice"}, projection={"_id": 0}).sort("created_at", -1).limit(200))
...
@app.get("/admin/user/<uid>/practicetests/<session_id>")
def admin_user_practicetest_detail(uid, session_id):
    """Full detail for a single practice test session (scores + analysis)."""
    _, err = require_admin()
    if err:
        msg, code = err
        return jsonify({"error": msg}), code
    ...
    db = get_db()
    col = _mocktest_sessions_col(db)
    doc = col.find_one({"uid": uid, "session_id": session_id, "kind": "practice"}, projection={"_id": 0})
    if not doc:
        doc = col.find_one({"uid": uid, "sessionId": session_id, "kind": "practice"}, projection={"_id": 0})
    if not doc:
        return jsonify({"ok": False, "error": "Not found"}), 404

    out = {
        "sessionId": doc.get("session_id") or doc.get("sessionId") or session_id,
        "title": doc.get("title") or "Practice Test",
        "mode": doc.get("mode") or "practice",
...
def _mocktest_generate_apti_questions(section: str, n: int, difficulty: str = "mixed", topic: str = ""):
    ...
    if topic:
        topic_hint = f"FOCUS TOPIC (STRICT): {topic}. Generate ALL questions only from this topic. Do NOT include other topics."
    else:
        topic_hint = f"Suggested topics: {topics}."
...
"""
    ...
    if topic:
        ...
        for q in qs:
            ...
            hay = " ".join([
                str(q.get("topic") or ""),
                str(q.get("subsection") or ""),
                str(q.get("question") or ""),
                " ".join(q.get("options") or []),
            ])
            if _topic_matches(hay, topic):
                filtered.append(q)
        qs = filtered
        ...
        if len(qs) < n:
            ...
            bank = topic_bank.get(key, [])
            i = 0
            while len(qs) < n and bank:
                b = dict(bank[i % len(bank)])
                b["id"] = f"{section.upper()}-T{len(qs)+1}"
                b["section"] = section
                b["difficulty"] = difficulty
                b["answerText"] = b["options"][int(b.get("correctIndex") or 0)]
                qs.append(b)
...
def _ensure_main_in_starter(code: str, lang: str) -> str:
    """Ensure starter templates include a runnable entrypoint.

    Some generator outputs return only a function stub. Zenith's editor should
    receive copy-paste runnable code by default.
    """
    ...
    if l == "sql":
        return code if isinstance(code, str) and code.strip() else _sql_starter_query()
...
def _starter_code_for_problem(problem: dict, lang: str) -> str:
    """Pick best starter code for this problem and language.

    Prefer problem['starterCode'] if present (from generator), else fall back to defaults.
    """
    if str((problem or {}).get("type") or "").strip().lower() == "sql":
        l = "sql"
    else:
        l = (lang or "python").strip().lower()
    sc = (problem or {}).get("starterCode") or (problem or {}).get("starter_code") or {}
    if isinstance(sc, dict):
        cand = sc.get(l)
        if isinstance(cand, str) and cand.strip():
            return _ensure_main_in_starter(cand, l)
        # fallbacks
        cand = sc.get("sql") or sc.get("python") or sc.get("java") or sc.get("cpp") or sc.get("c")
        if isinstance(cand, str) and cand.strip():
            return _ensure_main_in_starter(cand, l)
    defaults = _default_starter_code()
    return _ensure_main_in_starter(defaults.get(l) or defaults.get("python"), l)


def _mocktest_generate_coding_problems(n: int, difficulty: str = "mixed", topic: str = ""):
    ...
    topic_hint = f"Focus topic: {topic}." if topic else "Topic: Mix common DSA topics."

    prompt = (
        f"You are generating coding problems for a mock test platform.\n{topic_hint}\n"
        "Return STRICT JSON ONLY. No markdown.\n\n"
...
def _sql_rows_to_text(rows, columns=None):
    cols = [str(c) for c in (columns or [])]
    safe_rows = []
    for row in (rows or []):
        if isinstance(row, (list, tuple)):
            safe_rows.append(["" if v is None else str(v) for v in row])
        else:
            safe_rows.append(["" if row is None else str(row)])
    lines = []
    if cols:
...
def _normalize_sql_result_rows(rows):
    out = []
    for row in (rows or []):
...
def _execute_sql_query(setup_sql: str, query_sql: str):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    try:
        conn.executescript(setup_sql or "")
        cur = conn.cursor()
        cur.execute(query_sql or "")
        rows = _normalize_sql_result_rows(cur.fetchall())
...
def _build_sql_pandas_schema(schema_sql: str, sample_data_sql: str):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    try:
        conn.executescript((schema_sql or "") + "\n" + (sample_data_sql or ""))
        cur = conn.cursor()
        ...
        for idx, table in enumerate(tables):
            ...
            col_names = [str(r[1]) for r in pragma_rows]
            col_types = [str(r[2] or "").upper() for r in pragma_rows]
            rows = cur.execute(f'SELECT * FROM "{table}"').fetchall()
            data_rows = [list(r) for r in rows]
            var_name = ''.join(ch if ch.isalnum() else '_' for ch in table.lower()) or 'df'
            lines.append(f"data = {json.dumps(data_rows, ensure_ascii=False)}")
            lines.append(f"{var_name} = pd.DataFrame(data, columns={json.dumps(col_names, ensure_ascii=False)})")
            astype_map = {}
            for name, ctype in zip(col_names, col_types):
                ...
                for key, mapped in type_map.items():
                    if key in ctype:
                        ...
                        break
                if dtype:
                    astype_map[name] = dtype
            if astype_map:
                lines.append(f"{var_name} = {var_name}.astype({json.dumps(astype_map, ensure_ascii=False)})")
            if idx != len(tables) - 1:
                lines.append("")
        return "\n".join(lines).strip() + "\n"
    finally:
...
def _compare_sql_results(actual, expected):
    a_cols = [str(c) for c in (actual.get("columns") or [])]
    e_cols = [str(c) for c in (expected.get("columns") or [])]
    a_rows = actual.get("rows") or []
    e_rows = expected.get("rows") or []
    return a_cols == e_cols and a_rows == e_rows


def _sql_starter_query():
    return "-- Write your SQL query below\n-- Tables and data are preloaded for every test dataset.\n\nSELECT 1;\n"


def _mocktest_generate_sql_problems(n: int, difficulty: str = "mixed", topic: str = ""):
    n = max(1, min(40, int(n or 1)))
    difficulty = (difficulty or "mixed").strip().lower()
    topic_hint = _sanitize_topic(topic)
    ...
    plan = testcase_plan.get(difficulty, testcase_plan["mixed"])
    sample_need = int(plan["samples"])
    hidden_need = int(plan["hidden"])
    ...
    prompt = (
        ...
        f"Generate exactly {n} SQL problems. Difficulty: {difficulty}.\n"
        + (f"Focus topic: {topic_hint}.\n" if topic_hint else "")
        ...
        f"- sample_data_sqls must be an array of exactly {sample_need} INSERT-only SQL scripts for visible sample datasets.\n"
        f"- hidden_data_sqls must be an array of exactly {hidden_need} INSERT-only SQL scripts for hidden datasets.\n"
    ...
    raw = _gemini_json(prompt) or {}
    arr = raw.get("problems") if isinstance(raw, dict) else []
    out = []
    for i, p in enumerate(arr or [], start=1):
        if not isinstance(p, dict):
            continue
        schema_sql = str(p.get("schema_sql") or "").strip()
        sample_arr = p.get("sample_data_sqls") or []
        if not isinstance(sample_arr, list):
            sample_arr = []
        sample_arr = [str(x or "").strip() for x in sample_arr if str(x or "").strip()]
        if not sample_arr:
            legacy_sample = str(p.get("sample_data_sql") or "").strip()
            if legacy_sample:
                sample_arr = [legacy_sample]
        hidden_arr = p.get("hidden_data_sqls") or []
        hidden_arr = [str(x or "").strip() for x in hidden_arr if str(x or "").strip()]
        ref_sql = str(p.get("reference_sql") or "").strip()
        if not schema_sql or not sample_arr or not hidden_arr or not ref_sql:
            continue

        if len(sample_arr) < sample_need:
            sample_arr = (sample_arr * sample_need)[:sample_need]
        else:
            sample_arr = sample_arr[:sample_need]
        if len(hidden_arr) < hidden_need:
            hidden_arr = (hidden_arr * hidden_need)[:hidden_need]
        else:
            hidden_arr = hidden_arr[:hidden_need]

        datasets = []
        sample_previews = []
        pandas_seed_sql = sample_arr[0] if sample_arr else ""
        try:
            for idx, ssql in enumerate(sample_arr, start=1):
                sample_res = _execute_sql_query(schema_sql + "\n" + ssql, ref_sql)
                datasets.append({
                    "label": f"Sample dataset {idx}",
                    "setup_sql": ssql,
                    "expected_columns": sample_res.get("columns") or [],
                    "expected_rows": sample_res.get("rows") or [],
                    "hidden": False,
                })
                sample_previews.append({
                    "stdin": ssql,
                    "expected": _sql_rows_to_text(sample_res.get("rows") or [], sample_res.get("columns") or []),
                })
            for idx, hsql in enumerate(hidden_arr, start=1):
                hidden_res = _execute_sql_query(schema_sql + "\n" + hsql, ref_sql)
                datasets.append({
                    "label": f"Hidden dataset {idx}",
                    "setup_sql": hsql,
                    "expected_columns": hidden_res.get("columns") or [],
                    "expected_rows": hidden_res.get("rows") or [],
                    "hidden": True,
                })
        except Exception:
            continue
        pid = str(p.get("id") or f"sql_{i}_{uuid.uuid4().hex[:8]}")
        title = str(p.get("title") or f"SQL Problem {i}").strip() or f"SQL Problem {i}"
        ptopic = _sanitize_topic(str(p.get("topic") or topic_hint or "SQL")) or "SQL"
        full_statement = str(p.get("statement") or "").strip()
        full_statement += "\n\nReturn the final result as a SELECT query only."
        pdiff = str(p.get("difficulty") or difficulty or "medium").strip().lower() or "medium"
        plan2 = testcase_plan.get(pdiff, plan)
        out.append({
            "id": pid,
            "title": title,
            "difficulty": pdiff,
            ...
            "hiddenTests": [],
            "starterCode": {"sql": _sql_starter_query()},
            ...
            "total_marks": int(plan2["marks"]),
            "sql_meta": {
                "schema_sql": schema_sql,
                "pandas_schema": _build_sql_pandas_schema(schema_sql, pandas_seed_sql),
                "sample_datasets": [d for d in datasets if not d.get("hidden")],
                "hidden_datasets": [d for d in datasets if d.get("hidden")],
                "reference_sql": ref_sql,
            },
        })
    if not out:
        raise RuntimeError("Gemini did not return any valid SQL problems. Please regenerate.")
    return out
...
def _mocktest_public_session(doc: dict):
    ...
    return {
        ...
        "kind": doc.get("kind") or "mock",
        "pattern": doc.get("pattern") or {},
        "proctoring": _proctoring_public(doc.get("proctoring") or {}),
        "created_at": _ensure_iso_tz(doc.get("created_at")),
        "updated_at": _ensure_iso_tz(doc.get("updated_at")),
        ...
        "coding_problems": [
            ...
            | {"hiddenTestCount": len(((p or {}).get("hiddenTests") or [])) or len((((p or {}).get("sql_meta") or {}).get("hidden_datasets") or [])) or len(((p or {}).get("samples") or [])), "total_marks": (p or {}).get("total_marks")}
...
def mocktest_create_session():
    ...
    gen_n = max(0, min(40, int(pattern.get("general") or 0)))
    tech_n = max(0, min(40, int(pattern.get("tech") or 0)))
    coding_n = max(0, min(20, int(pattern.get("coding") or 0)))
    sql_n = max(0, min(20, int(pattern.get("sql") or 0)))

    if mode == "general":
        tech_n = 0; coding_n = 0; sql_n = 0
    elif mode == "tech":
        gen_n = 0; coding_n = 0; sql_n = 0
    elif mode == "coding":
        gen_n = 0; tech_n = 0; sql_n = 0
    elif mode == "sql":
        gen_n = 0; tech_n = 0
    else:
        mode = "all" 

    if gen_n + tech_n + coding_n + sql_n <= 0:
    ...
    try:
        ...
        requested_dsa_n = coding_n if mode in ["all", "coding"] else 0
        requested_sql_n = sql_n if mode == "all" else (coding_n if mode == "sql" else 0)
        dsa_probs = _mocktest_generate_coding_problems(requested_dsa_n, difficulty=difficulty) if requested_dsa_n else []
        sql_probs = _mocktest_generate_sql_problems(requested_sql_n, difficulty=difficulty, topic="SQL") if requested_sql_n else []
        coding_probs = (dsa_probs or []) + (sql_probs or [])
    except Exception as e:
    ...
    # Pre-fill coding drafts so editor is never empty on first open.
    # (Frontend also falls back to templates, but this guarantees immediate starter code.)
    coding_answers = {}
    for p in coding_probs or []:
        if not isinstance(p, dict):
            continue
        pid = p.get("id") or p.get("slug")
        if not pid:
            continue
        init_lang = "sql" if str((p.get("type") or "")).strip().lower() == "sql" else "python"
        coding_answers[str(pid)] = {
            "language": init_lang,
            "code": (_starter_code_for_problem(p, init_lang) or ""),
        }
    ...
    doc = {
        ...
        "mode": mode,
        "pattern": {"general": gen_n, "tech": tech_n, "coding": (coding_n if mode in ["all", "coding"] else 0), "sql": (sql_n if mode == "all" else (coding_n if mode == "sql" else 0))},
        "difficulty": difficulty,
        "kind": "mock",
        "status": "ready",
        ...
        "answers": {"general": {}, "tech": {}, "coding": coding_answers},
        "scores": {},
        "total_score": None,
        "total_marks": total_marks,
        "analysis": None,
        "proctoring": _default_proctoring(3),
        "created_at": datetime.utcnow(),
        "updated_at": datetime.utcnow(),
...
@app.post("/practice/session/create")
def practice_create_session():
    ...
    if section not in ["general", "tech", "dsa", "mixed", "sql"]:
        section = "general"
    difficulty = (payload.get("difficulty") or "easy").strip().lower()
    if difficulty not in ["easy", "mixed", "medium", "hard"]:
        difficulty = "easy"
    topic = _sanitize_topic(payload.get("topic") or "")
    count = int(payload.get("count") or 0)

    # Mixed practice can generate General + Tech + Coding + SQL together with per-area topics.
    topics = payload.get("topics") or {}
    counts = payload.get("counts") or {}

    if section == "mixed":
        gcount = max(0, min(40, int(counts.get("general") or 0)))
        tcount = max(0, min(40, int(counts.get("tech") or 0)))
        ccount = max(0, min(20, int(counts.get("coding") or 0)))
        scount = max(0, min(20, int(counts.get("sql") or 0)))
        if gcount + tcount + ccount + scount <= 0:
            return jsonify({"error": "Please enter at least one question/problem count."}), 400
    elif section == "dsa":
        count = max(1, min(20, count or 1))
    elif section == "sql":
        count = max(1, min(20, count or 1))
    else:
        count = max(1, min(40, count or 1))

    try:
        if section == "mixed":
            gtopic = _sanitize_topic(str(topics.get("general") or ""))
            ttopic = _sanitize_topic(str(topics.get("tech") or ""))
            ctopic = _sanitize_topic(str(topics.get("coding") or ""))
            stopic = _sanitize_topic(str(topics.get("sql") or ""))
            general_qs = _mocktest_generate_apti_questions("general", gcount, difficulty=difficulty, topic=gtopic) if gcount else []
            tech_qs = _mocktest_generate_apti_questions("tech", tcount, difficulty=difficulty, topic=ttopic) if tcount else []
            dsa_probs = _mocktest_generate_coding_problems(ccount, difficulty=difficulty, topic=ctopic) if ccount else []
            sql_probs = _mocktest_generate_sql_problems(scount, difficulty=difficulty, topic=stopic or "SQL") if scount else []
            coding_probs = (dsa_probs or []) + (sql_probs or [])
            pattern = {"general": gcount, "tech": tcount, "coding": ccount, "sql": scount}
            mode = "all"
            # store structured topics for mixed
            topic = {"general": gtopic, "tech": ttopic, "coding": ctopic, "sql": stopic}
        elif section in ["general", "tech"]:
            generated_qs = _mocktest_generate_apti_questions(section, count, difficulty=difficulty, topic=topic) if count else []
            general_qs = generated_qs if section == "general" else []
            tech_qs = generated_qs if section == "tech" else []
            coding_probs = []
            pattern = {"general": count if section == "general" else 0, "tech": count if section == "tech" else 0, "coding": 0, "sql": 0}
            mode = section
        elif section == "dsa":
            general_qs = []
            tech_qs = []
            coding_probs = _mocktest_generate_coding_problems(count, difficulty=difficulty, topic=topic) if count else []
            pattern = {"general": 0, "tech": 0, "coding": count, "sql": 0}
            mode = "coding"
        else:
            general_qs = []
            tech_qs = []
            coding_probs = _mocktest_generate_sql_problems(count, difficulty=difficulty, topic=topic or "SQL") if count else []
    ...
    doc = {
        "uid": user.get("uid"),
        "session_id": str(uuid.uuid4()),
        "title": (payload.get("title") or "Practice Test").strip() or "Practice Test",
        "mode": mode,
        "pattern": pattern,
        "difficulty": difficulty,
        "kind": "practice",
        ...
        "answers": {
            ...
            "coding": {
                str((p.get("id") or p.get("slug"))): {
                    "language": ("sql" if str((p.get("type") or "")).strip().lower() == "sql" else "python"),
                    "code": _starter_code_for_problem(p, ("sql" if str((p.get("type") or "")).strip().lower() == "sql" else "python")) or "",
                }
                for p in (coding_probs or [])
    ...
    return jsonify({"ok": True, "session": {"session_id": doc["session_id"]}})
...
def mocktest_list_sessions():
    ...
    q = {"uid": uid}
    # Backward compat: older mock sessions may not have the "kind" field.
    if kind == "practice":
        q["kind"] = "practice"
    elif kind == "mock":
        q["$or"] = [
    ...
    col = _mocktest_sessions_col(get_db())
    # Show more history (users asked for full history)
    items = list(col.find(q).sort("created_at", -1).limit(200))
    # public summary list
    out = []
    for d in items:
        out.append({
            ...
            "created_at": _ensure_iso_tz(d.get("created_at")),
            "updated_at": _ensure_iso_tz(d.get("updated_at")),
            "total_score": d.get("total_score"),
            "total_marks": d.get("total_marks"),
...
@app.post("/mocktest/sessions/<session_id>/save")
def mocktest_save_progress(session_id):
    """Persist in-progress answers (MCQ choices + code drafts) so refresh doesn't reset."""
    user, err = require_user()
    if err:
        return jsonify({"error": err[0]}), err[1]
    uid = user.get("uid")
    ...
    if not doc:
        return jsonify({"error": "Not found"}), 404
    if doc.get("submittedAt"):
        return jsonify({"error": "Already submitted"}), 400

    # Merge (shallow) to avoid wiping other sections.
    cur = doc.get("answers") or {}
    merged = dict(cur)
    for k, v in (answers or {}).items():
        if isinstance(v, dict):
            merged[k] = {**(cur.get(k) or {}), **v}
        else:
            merged[k] = v

    col.update_one(
        {"uid": uid, "session_id": session_id},
        {"$set": {"answers": merged, "updated_at": utc_iso()}},
    )
    return jsonify({"ok": True})
...
def mocktest_code_run():
    ...
    results = []
    if (payload.get("problem_type") or "").strip().lower() == "sql" or language == "sql":
        for t in tests[:4]:
            if not isinstance(t, dict):
                continue
            schema_sql = str(payload.get("schema_sql") or t.get("schema_sql") or "")
            setup_sql = str(t.get("setup_sql") or "")
            expected = {"columns": t.get("expected_columns") or [], "rows": t.get("expected_rows") or []}
            try:
                actual = _execute_sql_query((schema_sql + "\n" + setup_sql).strip(), source_code)
                passed = _compare_sql_results(actual, expected)
                r = {
                    "stdout": _sql_rows_to_text(actual.get("rows") or [], actual.get("columns") or []),
                    "stderr": "",
                    "compile_output": "",
                    "status": "Accepted" if passed else "Wrong Answer",
                    "status_id": 3 if passed else 4,
                    "expected": _sql_rows_to_text(expected.get("rows") or [], expected.get("columns") or []),
            ...
            r["stdin"] = ((schema_sql + "\n" + setup_sql).strip() if schema_sql else setup_sql)
            results.append(r)
    else:
        for t in tests[:4]:
            if not isinstance(t, dict):
    ...
    analysis = {} if ((payload.get("problem_type") or "").strip().lower() == "sql" or language == "sql") else _gemini_code_analysis(payload.get("problem_text") or payload.get("problem") or "", source_code, language)
    ...
    for i, r in enumerate(results):
        ...
        testcases.append({
            ...
            "expected_columns": r.get("expected_columns") or [],
            "expected_rows": r.get("expected_rows") or [],
            "actual_columns": r.get("actual_columns") or [],
            "actual_rows": r.get("actual_rows") or [],
            "is_sql": bool(r.get("is_sql")),
        })
...
def mocktest_submit_single_coding(session_id, pid):
    ...
    is_sql_problem = str((prob or {}).get("type") or "").strip().lower() == "sql"
    tests_samples = ((prob.get("sql_meta") or {}).get("sample_datasets") or []) if is_sql_problem else (prob.get("samples") or [])
    tests_hidden = ((prob.get("sql_meta") or {}).get("hidden_datasets") or []) if is_sql_problem else (prob.get("hiddenTests") or [])
    ...
    tests_samples_n = tests_samples if is_sql_problem else _norm_tests(tests_samples)
    tests_hidden_n = tests_hidden if is_sql_problem else _norm_tests(tests_hidden)
    ...
    for i, t in enumerate(tests, start=1):
        ...
        try:
            if is_sql_problem:
                setup_sql = t.get("setup_sql") or ""
                expected_obj = {"columns": t.get("expected_columns") or [], "rows": t.get("expected_rows") or []}
                actual = _execute_sql_query((prob.get("sql_meta") or {}).get("schema_sql") + "\n" + setup_sql, source_code)
                passed_sql = _compare_sql_results(actual, expected_obj)
                r = {
                    "stdout": _sql_rows_to_text(actual.get("rows") or [], actual.get("columns") or []),
                    "stderr": "",
                    "compile_output": "",
                    "status": "Accepted" if passed_sql else "Wrong Answer",
                    "status_id": 3 if passed_sql else 4,
                    "expected": _sql_rows_to_text(expected_obj.get("rows") or [], expected_obj.get("columns") or []),
                    "expected_columns": expected_obj.get("columns") or [],
                    "expected_rows": expected_obj.get("rows") or [],
                    "actual_columns": actual.get("columns") or [],
                    "actual_rows": actual.get("rows") or [],
                    "is_sql": True,
                    "passed": passed_sql,
            ...
            if is_sql_problem:
                r["expected_columns"] = (expected_obj.get("columns") or []) if isinstance(locals().get("expected_obj"), dict) else []
                r["expected_rows"] = (expected_obj.get("rows") or []) if isinstance(locals().get("expected_obj"), dict) else []
                r["actual_columns"] = []
                r["actual_rows"] = []
                r["is_sql"] = True
    ...
    summary = {
        ...
        "timeComplexity": None if is_sql_problem else (prob.get("timeComplexity") or prob.get("time_complexity") or (tc_analysis.get("timeComplexity") if isinstance(tc_analysis, dict) else None)),
        "spaceComplexity": None if is_sql_problem else (prob.get("spaceComplexity") or prob.get("space_complexity") or (tc_analysis.get("spaceComplexity") if isinstance(tc_analysis, dict) else None)),
        "analysis": None if is_sql_problem else (tc_analysis.get("analysis") if isinstance(tc_analysis, dict) else None),
    }
    ...
    try:
        ...
        total_marks = c_total_marks if mode2 in ["coding", "sql"] else base_q + c_total_marks
...
@app.post("/mocktest/sessions/<session_id>/proctoring/violation")
def mocktest_proctoring_violation(session_id):
    user, err = require_user()
    if err:
        return jsonify({"error": err[0]}), err[1]
    uid = user.get("uid")
    payload = request.get_json(silent=True) or {}
    event_type = (payload.get("type") or payload.get("event") or "screen-switch").strip() or "screen-switch"
    ...
    kind = (doc.get("kind") or "mock").strip().lower()
    if kind == "practice":
        return jsonify({"error": "Proctoring is enabled only for mock tests."}), 400

    current = doc.get("proctoring") or _default_proctoring(3)
    limit = int(current.get("violation_limit") or 3)

    if (doc.get("status") or "").lower() == "submitted":
        return jsonify({"ok": True, "proctoring": _proctoring_public(current), "auto_submitted": bool(current.get("auto_submitted"))})
    ...
    warnings = list(current.get("warnings") or []) + [warning]
    events = list(current.get("events") or []) + [event]
    auto_submitted = new_count >= limit

    proctoring = {
        **_default_proctoring(limit),
        **current,
        "enabled": True,
        "violation_limit": limit,
        ...
        "warnings": warnings,
        "events": events,
        "last_violation_at": now_iso,
        "auto_submitted": auto_submitted,
        "auto_submit_reason": ("Auto-submitted due to proctoring violation" if auto_submitted else current.get("auto_submit_reason") or ""),
    }
    ...
    if auto_submitted:
        total_marks = int(doc.get("total_marks") or 0)
        zero_scores = {
            "general": 0,
            "general_total": int((doc.get("pattern") or {}).get("general") or len(doc.get("general_questions") or [])),
            "tech": 0,
            "tech_total": int((doc.get("pattern") or {}).get("tech") or len(doc.get("tech_questions") or [])),
            "coding": 0,
            "coding_total": int(sum(int((p or {}).get("total_marks") or 0) for p in (doc.get("coding_problems") or []) if isinstance(p, dict))),
        }
        set_fields.update({
            "status": "submitted",
            "submittedAt": now_iso,
    ...
    col.update_one({"_id": doc.get("_id")}, {"$set": set_fields})

    if auto_submitted:
        try:
            to_email = (user.get("email") or "").strip()
            if to_email:
                subj = "Zenith Mock Test Auto-Submitted Due to Proctoring Violation"
                html = _brand_email(
                    title="Mock test auto-submitted",
                    subtitle="Proctoring violation detected",
                    body_html=(
                        f"<p>Your mock test <b>{_html_escape(doc.get('title') or 'Mock Test')}</b> was auto-submitted with <b>0 marks</b>.</p>"
                        f"<p><b>Violation count:</b> {new_count}/{limit}</p>"
                        f"<p><b>Last violation time:</b> {_html_escape(_fmt_ist(now_dt))}</p>"
                        f"<p>Please stay on the Zenith mock test screen during future attempts.</p>"
                        + _proctoring_details_html(proctoring)
                    ),
                    cta_url="/my-tests?tab=mock",
                    cta_text="Open Mock Test History",
                    kind="mocktest",
                )
                _send_email(
                    to_email,
                    ...
                    html,
                    text_body=(
                        f"Your mock test '{doc.get('title') or 'Mock Test'}' was auto-submitted with 0 marks due to proctoring violations.\n"
                        f"Violation count: {new_count}/{limit}\n"
                        f"Last violation time: {_fmt_ist(now_dt)}\n"
                        + _proctoring_details_text(proctoring)
                    ),
                    kind="mocktest",
                    reply_to=(os.getenv("CONTACT_INBOX") or os.getenv("ADMIN_EMAIL")),
                )
            ...
            try:
                print("[email] proctoring violation send failed:", repr(e))
            except Exception:
                pass

    return jsonify({
        "ok": True,
        "proctoring": _proctoring_public(proctoring),
        "auto_submitted": auto_submitted,
        "submitted": auto_submitted,
        "total_score": 0 if auto_submitted else None,
        "total_marks": int(doc.get("total_marks") or 0),
    })
...
def mocktest_submit(session_id):
    ...
    for p in problems:
        ...
        # If user submits without typing anything, still evaluate using the starter template
        # so the judge runs and we can show proper failures.
        if not str(code).strip():
            code = _starter_code_for_problem(p, lang or "python") or ""
        ...
        is_sql_problem = str((p or {}).get("type") or "").strip().lower() == "sql"
        samples = ((p.get("sql_meta") or {}).get("sample_datasets") or []) if is_sql_problem else (p.get("samples") or [])
        hidden = ((p.get("sql_meta") or {}).get("hidden_datasets") or []) if is_sql_problem else (p.get("hiddenTests") or [])
        ...
        for idx, t in enumerate(tests):
            try:
                if is_sql_problem:
                    actual = _execute_sql_query(((p.get("sql_meta") or {}).get("schema_sql") or "") + "\n" + (t.get("setup_sql") or ""), code)
                    expected_obj = {"columns": t.get("expected_columns") or [], "rows": t.get("expected_rows") or []}
                    ok_sql = _compare_sql_results(actual, expected_obj)
                    jr = {
                        "passed": ok_sql,
                        "stdout": _sql_rows_to_text(actual.get("rows") or [], actual.get("columns") or []),
                        "expected": _sql_rows_to_text(expected_obj.get("rows") or [], expected_obj.get("columns") or []),
                        "status": "Accepted" if ok_sql else "Wrong Answer",
                    }
                else:
...
"""
    ...
    try:
        ...
        if to_email:
            ...
            test_name = _sanitize_test_name_for_email(test_name, is_practice=(doc.get("kind") or "mock").strip().lower() == "practice")
            ...
            test_kind = (doc.get("kind") or "mock").strip().lower()
            is_practice = test_kind == "practice"
            report_title = "Zenith Practice Test Report" if is_practice else "Zenith Mock Test Report"
            cta_text = "View Practice Test History" if is_practice else "View Mock Test History"
            cta_url = "/my-tests?tab=practice" if is_practice else "/my-tests?tab=mock"

            proctoring_block_html = "" if is_practice else _proctoring_details_html(doc.get("proctoring") or {})
            proctoring_block_text = "" if is_practice else _proctoring_details_text(doc.get("proctoring") or {})

            body_html = (
                ("<p>Your practice test has been evaluated and your report is ready ✅</p>" if is_practice else "<p>Your mock test has been evaluated and your report is ready ✅</p>")
            ...
            html = _brand_email(
                ...
                cta_url=cta_url,
                cta_text=cta_text,
                kind=("practicetest" if is_practice else "mocktest"),
            )
...
def _case_to_public_dict(case, hide_io=False):
    ...
    if hide_io:
        ...
        d["stderr"] = (case.get("stderr") or "")
        d["expected_columns"] = (case.get("expected_columns") or [])
        d["expected_rows"] = (case.get("expected_rows") or [])
        d["actual_columns"] = (case.get("actual_columns") or [])
        d["actual_rows"] = (case.get("actual_rows") or [])
    d["is_sql"] = bool(case.get("is_sql"))
    return d
if __name__ == "__main__":
    app.run(debug=True)